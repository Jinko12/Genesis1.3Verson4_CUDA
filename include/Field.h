...
class Field{
 public:
   ...
   double getRHarm();
#ifdef GENESIS_CUDA
   bool syncCudaFieldToHost();
   void markCudaHostFieldDirty();
   bool copyCudaFieldToBeam(genesis_cuda::BeamState *, std::size_t);
   bool canBuildSourceFromCudaBeam() const;
   bool applyCudaSlippage(int direction);
   bool downloadCudaSlippageSlice(int direction, double *hostBuffer, std::size_t hostDoubles);
   bool applyCudaSlippageBoundary(int direction, const double *hostBoundary, std::size_t hostDoubles, bool zeroBoundary);
#endif
