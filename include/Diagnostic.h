...
#ifdef GENESIS_CUDA
    bool getCudaValues(Beam *, std::map<std::string,std::vector<double> > &, int) override;
#endif
};
...
#ifdef GENESIS_CUDA
    bool getCudaValues(Field *, std::map<std::string,std::vector<double> > &, int) override;
#endif
...
private:
    ...
    std::vector<bool> activeBeamDiag_;
    std::vector<bool> activeFieldDiag_;
