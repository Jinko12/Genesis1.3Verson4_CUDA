...
#ifdef GENESIS_CUDA
    virtual bool getCudaValues(Beam *, std::map<std::string,std::vector<double> > &, int) { return false; }
#endif
};
...
#ifdef GENESIS_CUDA
    virtual bool getCudaValues(Field *, std::map<std::string,std::vector<double> > &, int) { return false; }
#endif
};
