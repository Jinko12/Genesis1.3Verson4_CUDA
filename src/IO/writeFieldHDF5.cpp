...
{
  GENESIS_NVTX_RANGE("hdf5.write_field_all");
  string file;
...
{
  GENESIS_NVTX_RANGE("hdf5.write_field_one");
#ifdef GENESIS_CUDA
  if ((field != nullptr) && !field->syncCudaFieldToHost()) {
    if (rank == 0) {
      cout << "Failed to synchronize CUDA field state before field dump." << endl;
    }
    return(false);
  }
#endif
