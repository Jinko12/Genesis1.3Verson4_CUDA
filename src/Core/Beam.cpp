...
int Beam::sort()
{
  GENESIS_NVTX_RANGE("beam.sort");
#ifdef GENESIS_CUDA
  solver.syncCudaTrackingToHost(this);
  solver.invalidateCudaCache();
#endif
  int shift=0;
...
void Beam::setCudaTracking(bool enabled) {
  solver.setCudaTracking(enabled);
}

#ifdef GENESIS_CUDA
void Beam::syncCudaTrackingToHost() {
  solver.syncCudaTrackingToHost(this);
}

void Beam::invalidateCudaTrackingCache() {
  solver.invalidateCudaCache();
}

genesis_cuda::BeamState *Beam::cudaBeamState() {
  return solver.cudaBeamState();
}

std::size_t Beam::cudaBeamParticleCount() const {
  return solver.cudaBeamParticleCount();
}

bool Beam::cudaBeamStateAvailable() const {
  return solver.cudaBeamStateAvailable();
}
#endif

void Beam::track(double delz,vector<Field *> *field, Undulator *und){
  GENESIS_NVTX_RANGE("beam.track");
  ...
  {
    GENESIS_NVTX_RANGE("beam.track.transverse_half_1");
    solver.track(delz*0.5,this,und,false);   // track transverse coordinates first half of integration step
  }

  {
    GENESIS_NVTX_RANGE("beam.track.longitudinal");
    solver.advance(delz,this,field,und);     // advance longitudinal variables
  }

  // The collective/incoherent modules are still CPU-owned.  Synchronize only
  // when at least one of them can actually mutate the beam; otherwise keep the
  // device-resident particle state alive through R56 and the second transverse
  // half step.
  const bool needsCpuOwnedBeam = (incoherent.isEnabled() && und->inUndulator()) || col.hasWakeDefined();
  if (needsCpuOwnedBeam) {
    GENESIS_NVTX_RANGE("beam.track.cpu_owned_physics");
    solver.syncCudaTrackingToHost(this);
  ...
  {
    GENESIS_NVTX_RANGE("beam.track.r56");
    solver.applyR56(this,und,reflength);    // apply the longitudinal phase shift due to R56 if a chicane is selected.
  }

  {
    GENESIS_NVTX_RANGE("beam.track.transverse_half_2");
    solver.track(delz*0.5,this,und,true);
  }

  bool syncBeamAfterStep = true;
#ifdef GENESIS_CUDA
  // If every enabled field solver can consume the device-resident Beam SoA for
  // source deposition, keep the beam on the GPU until the next explicit CPU
  // boundary (sorting, diagnostics, dumps, CPU fallback, or CPU-only physics).
  if (solver.cudaTrackingEnabled() && (field != nullptr) && !field->empty()) {
    syncBeamAfterStep = false;
    for (auto *ifld : *field) {
      if ((ifld == nullptr) || !ifld->canBuildSourceFromCudaBeam()) {
        syncBeamAfterStep = true;
        break;
      }
    }
  }
#endif
  if (syncBeamAfterStep) {
    GENESIS_NVTX_RANGE("beam.track.final_host_sync");
    solver.syncCudaTrackingToHost(this);
  }
}
