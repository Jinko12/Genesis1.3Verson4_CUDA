...
bool cudaDiagnosticReductionEnabled() {
    const char *env = std::getenv("GENESIS_CUDA_DIAG_REDUCTION");
...
void Diagnostic::init(int rank, int size, int nz_in, int ns_in, int nfld,bool isTime, bool isScan, FilterDiagnostics &filter)
{
 ...
 //   FilterDiagnostics filter;
    ...
    units.clear();
    single.clear();
    activeBeamDiag_.clear();
    activeFieldDiag_.clear();
    ...
    for (auto group : dbeam){
        const auto tags = group->getTags(filter);
        activeBeamDiag_.push_back(!tags.empty());
    ...
    for (int ifld=0; ifld<nfld;ifld++){
        ...
        for (auto group : dfield){
            const auto tags = group->getTags(filter);
            if (ifld == 0) {
                activeFieldDiag_.push_back(!tags.empty());
            }
            for (auto const &tag : tags){
...
{
    GENESIS_NVTX_RANGE("diagnostics.write_to_hdf5");
    // lock the vectors holding the instances of diagnostic classes
    diag_can_add=false;
...
void Diagnostic::calc(Beam* beam,std::vector<Field*> *field,double z) {
    GENESIS_NVTX_RANGE("diagnostics.calc");
    // lock the vectors holding the instances of diagnostic classes
    diag_can_add=false;

    zout[iz]=z;

    bool beamHostSynced = false;
    for (std::size_t igroup = 0; igroup < dbeam.size(); ++igroup) {
        if ((igroup < activeBeamDiag_.size()) && !activeBeamDiag_[igroup]) {
            continue;
        }
        bool computedOnCuda = false;
#ifdef GENESIS_CUDA
        if (cudaDiagnosticReductionEnabled()) {
            GENESIS_NVTX_RANGE("diagnostics.beam_cuda_reduction_try");
            computedOnCuda = dbeam[igroup]->getCudaValues(beam, val[2], iz);
        }
        if (!computedOnCuda && !beamHostSynced && (beam != nullptr)) {
            beam->syncCudaTrackingToHost();
            beamHostSynced = true;
        }
#endif
        if (!computedOnCuda) {
            GENESIS_NVTX_RANGE("diagnostics.beam_cpu_fallback");
            dbeam[igroup]->getValues(beam,val[2],iz);
        }
    }

    for (int ifld=0; ifld < field->size(); ifld++){
        bool fieldHostSynced = false;
        for (std::size_t igroup = 0; igroup < dfield.size(); ++igroup) {
            if ((igroup < activeFieldDiag_.size()) && !activeFieldDiag_[igroup]) {
                continue;
            }
            bool computedOnCuda = false;
#ifdef GENESIS_CUDA
            if (cudaDiagnosticReductionEnabled()) {
                GENESIS_NVTX_RANGE("diagnostics.field_cuda_reduction_try");
                computedOnCuda = dfield[igroup]->getCudaValues(field->at(ifld), val[3+ifld], iz);
            }
            if (!computedOnCuda && !fieldHostSynced) {
                Field *fld = field->at(ifld);
                if ((fld != nullptr) && !fld->syncCudaFieldToHost()) {
                    std::cerr << "[Genesis CUDA] Cannot synchronize CUDA field before diagnostics." << std::endl;
                    MPI_Abort(MPI_COMM_WORLD,1);
                }
                fieldHostSynced = true;
            }
#endif
            if (!computedOnCuda) {
                GENESIS_NVTX_RANGE("diagnostics.field_cpu_fallback");
                dfield[igroup]->getValues(field->at(ifld),val[3+ifld],iz);
            }
        }
    }
    iz++;
...
#ifdef GENESIS_CUDA
bool DiagBeam::getCudaValues(Beam *beam,std::map<std::string,std::vector<double> >&val, int iz)
{
    GENESIS_NVTX_RANGE("kernel.diagnostics_beam_compact");
    if ((beam == nullptr) || !beam->cudaBeamStateAvailable()) {
        return false;
    }

    const int ns = static_cast<int>(beam->beam.size());
    if (ns < 0) {
        return false;
    }
    std::vector<genesis_cuda::BeamSliceDiagnostic> diag(static_cast<std::size_t>(ns));
    std::vector<complex<double> > bsum(static_cast<std::size_t>(std::max(1u, nharm)) * static_cast<std::size_t>(ns));

    if (!genesis_cuda::beamComputeSliceDiagnostics(beam->cudaBeamState(),
                                                   static_cast<int>(nharm),
                                                   diag.data(),
                                                   bsum.data())) {
        return false;
    }
    ...
    for (int is = 0; is < ns; ++is) {
        const auto &d = diag[static_cast<std::size_t>(is)];
        double norm = 1.0;
        if (d.count > 0) {
            norm = 1.0 / static_cast<double>(d.count);
        }

        const double x1 = d.x1 * norm;
        const double x2 = d.x2 * norm;
        const double y1 = d.y1 * norm;
        const double y2 = d.y2 * norm;
        const double px1 = d.px1 * norm;
        const double py1 = d.py1 * norm;
        const double px2 = d.px2 * norm;
        const double py2 = d.py2 * norm;
        const double g1 = d.g1 * norm;
        const double g2 = d.g2 * norm;
        const double xpx = d.xpx * norm;
        const double ypy = d.ypy * norm;
        ...
        const complex<double> b0 = bsum[static_cast<std::size_t>(is) * nharm] * norm;
        this->storeValue(val, "bunching", idx, std::abs(b0));
        this->storeValue(val, "bunchingphase", idx, atan2(b0.imag(), b0.real()));
        char buff[100];
        if (exclharm && (nharm>1)){
            const complex<double> bh = bsum[static_cast<std::size_t>(is) * nharm + (nharm - 1)] * norm;
            snprintf(buff, sizeof(buff), "bunching%d", nharm);
            this->storeValue(val, buff, idx, std::abs(bh));
            snprintf(buff, sizeof(buff), "bunchingphase%d", nharm);
            this->storeValue(val, buff, idx, atan2(bh.imag(), bh.real()));
        } else {
            for (int iharm = 1; iharm < static_cast<int>(nharm); iharm++) {
                const complex<double> bh = bsum[static_cast<std::size_t>(is) * nharm + static_cast<std::size_t>(iharm)] * norm;
                snprintf(buff, sizeof(buff), "bunching%d", iharm + 1);
                this->storeValue(val, buff, idx, std::abs(bh));
                snprintf(buff, sizeof(buff), "bunchingphase%d", iharm + 1);
                this->storeValue(val, buff, idx, atan2(bh.imag(), bh.real()));
        ...
        if (filter["aux"]) {
            ...
            this->storeValue(val,"LSCfield",idx,beam->longESC[is]);
            this->storeValue(val,"SSCfield",idx,beam->getSCField(is));
            this->storeValue(val,"xmin",idx,d.xmin);
            this->storeValue(val,"xmax",idx,d.xmax);
            this->storeValue(val,"pxmin",idx,d.pxmin);
            this->storeValue(val,"pxmax",idx,d.pxmax);
            this->storeValue(val,"ymin",idx,d.ymin);
            this->storeValue(val,"ymax",idx,d.ymax);
            this->storeValue(val,"pymin",idx,d.pymin);
            this->storeValue(val,"pymax",idx,d.pymax);
            this->storeValue(val,"emin",idx,d.gmin);
            this->storeValue(val,"emax",idx,d.gmax);
        }
        if (tags["current"].once){
            if (iz ==0) {
                this->storeValue(val,"current",idx,beam->current[is]);
    ...
    if (global){
        ...
        if (size>1){
            double temp;
            MPI_Allreduce(&g_cur, &temp, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD); g_cur=temp;
            MPI_Allreduce(&g_g1, &temp, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD); g_g1=temp;
            MPI_Allreduce(&g_g2, &temp, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD); g_g2=temp;
            MPI_Allreduce(&g_x1, &temp, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD); g_x1=temp;
            MPI_Allreduce(&g_x2, &temp, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD); g_x2=temp;
            MPI_Allreduce(&g_y1, &temp, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD); g_y1=temp;
            MPI_Allreduce(&g_y2, &temp, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD); g_y2=temp;
        }
        double norm =1;
        if (g_cur>0){norm=1./g_cur;}
...
#ifdef GENESIS_CUDA
bool DiagField::getCudaValues(Field *field,std::map<std::string,std::vector<double> >&val, int iz){
    GENESIS_NVTX_RANGE("kernel.diagnostics_field_compact");
    if (field == nullptr) {
        return false;
    }
    const bool includeFftMoments = filter["fft"];

    genesis_cuda::CachedFFTFieldView view;
    if (!FieldSolverFFTCuda::getCachedFieldView(field, &view)) {
        return false;
    }

    const int ns = static_cast<int>(field->field.size());
    const int ngrid = field->ngrid;
    if ((ns <= 0) || (view.batchSize != static_cast<std::size_t>(ns)) ||
        (view.ngrid != static_cast<unsigned int>(ngrid))) {
        return false;
    }

    std::vector<genesis_cuda::FieldSliceDiagnostic> diag(static_cast<std::size_t>(ns));
    if (!genesis_cuda::fftFieldComputeSliceDiagnostics(view.state,
                                                       view.ngrid,
                                                       view.batchSize,
                                                       includeFftMoments,
                                                       diag.data())) {
    ...
    double f_y1=0;
    double f_y2=0;
    const double scltheta=field->xlambda/ngrid/field->dgrid;
    ...
    for (int is = 0; is < ns; ++is) {
        ...
        const double inten = d.centerRe * d.centerRe + d.centerIm * d.centerIm;
        const double intenphi = atan2(d.centerIm,d.centerRe);
        double farfield = d.ffRe * d.ffRe + d.ffIm * d.ffIm;
        const double farfieldphi = atan2(d.ffIm,d.ffRe);
        farfield *= field->dgrid * field->dgrid;
        if (global){
            g_ff+=farfield;
            g_inten+=inten;
        ...
        double powerScaled = power*scl*scl/vacimp;
        x1*=field->dgrid;
        x2*=field->dgrid;
        y1*=field->dgrid;
        y2*=field->dgrid;
        double intenScaled=inten*eev*eev/ks/ks/vacimp;

        int idx = iz*ns+is;
        this->storeValue(val,"power",idx,powerScaled);
        if (filter["spatial"]){
            this->storeValue(val,"xposition",idx,x1);
            this->storeValue(val,"xsize",idx,x2);
            this->storeValue(val,"yposition",idx,y1);
        ...
        if (filter["intensity"]){
            this->storeValue(val,"intensity-nearfield",idx,intenScaled);
            this->storeValue(val,"phase-nearfield",idx,intenphi);
            this->storeValue(val,"intensity-farfield",idx,farfield);
            this->storeValue(val,"phase-farfield",idx,farfieldphi);
        }
    ...
    if (global) {
        ...
        if (size > 1) {
            double temp;
            MPI_Allreduce(&g_pow, &temp, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD); g_pow = temp;
            MPI_Allreduce(&g_x1, &temp, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD); g_x1 = temp;
            MPI_Allreduce(&g_x2, &temp, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD); g_x2 = temp;
            MPI_Allreduce(&g_y1, &temp, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD); g_y1 = temp;
            MPI_Allreduce(&g_y2, &temp, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD); g_y2 = temp;
            MPI_Allreduce(&g_ff, &temp, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD); g_ff = temp;
            MPI_Allreduce(&g_inten, &temp, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD); g_inten = temp;
            if (includeFftMoments) {
                MPI_Allreduce(&f_pow, &temp, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD); f_pow = temp;
                MPI_Allreduce(&f_x1, &temp, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD); f_x1 = temp;
                MPI_Allreduce(&f_x2, &temp, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD); f_x2 = temp;
                MPI_Allreduce(&f_y1, &temp, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD); f_y1 = temp;
                MPI_Allreduce(&f_y2, &temp, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD); f_y2 = temp;
            }
        }
        double norm = 1;
        if (g_pow > 0) { norm = 1. / g_pow; }
        g_x1 *= norm;
        g_x2 *= norm;
        g_y1 *= norm;
        g_y2 *= norm;
