...
{
  GENESIS_NVTX_RANGE("beam.cpu.track_transverse_core");
...
#ifdef GENESIS_CUDA
bool TrackBeam::trackCuda(double delz,
                          Beam *beam,
                          Undulator *und,
                          bool lastStep,
                          genesis_cuda::BeamState *state,
                          size_t npart)
{
  GENESIS_NVTX_RANGE("beam.cuda.track_transverse_setup");
  (void)beam;
  ...
  double betpar0=sqrt(1-(1+aw*aw)/gamma0/gamma0);
  double qquad=qf*gamma0;
  double qnatx=kx*aw*aw/gamma0/betpar0;
  double qnaty=ky*aw*aw/gamma0/betpar0;
  ...
  if (lastStep){
    if ((cx!=0) || (cy!=0)) {
      GENESIS_NVTX_RANGE("kernel.beam_corrector");
      if (!genesis_cuda::beamApplyCorrector(state,npart,cx*gamma0,cy*gamma0)) {
        return false;
      }
    }
  } else {
    if (angle!=0) {
      ...
      GENESIS_NVTX_RANGE("kernel.beam_chicane_matrix");
      if (!genesis_cuda::beamApplyChicaneMatrix(state,npart,flat)) {
        return false;
      }
    }
  }
  ...
  {
    GENESIS_NVTX_RANGE("kernel.beam_transverse_core");
    return genesis_cuda::beamTrackTransverse(state,npart,delz,aw,qx,qy,xoff,yoff,modeX,modeY);
  }
}

bool TrackBeam::applyR56Cuda(Beam *beam,
                             Undulator *und,
                             double lambda0,
                             genesis_cuda::BeamState *state,
                             size_t npart)
{
  GENESIS_NVTX_RANGE("beam.cuda.r56_setup");
  (void)beam;
  double angle,lb,ld,lt;
  ...
  {
    GENESIS_NVTX_RANGE("kernel.beam_r56_core");
    return genesis_cuda::beamApplyR56(state,npart,R56,gamma0);
  }
}
#endif
