...
{
    GENESIS_NVTX_RANGE("gencore.run");
    ...
    {
      GENESIS_NVTX_RANGE("diagnostics.initial_calc");
	...
	{
	  GENESIS_NVTX_RANGE("main_loop.step");
	  ...
	  bool sort=false;
	  {
	    GENESIS_NVTX_RANGE("main_loop.apply_marker");
	    sort=control->applyMarker(beam, field, und, error_IO);
	  }
	  if(error_IO) {
	    return(false);
	  }
	  ...
	  {
	    GENESIS_NVTX_RANGE("main_loop.beam_track");
	  ...
	  if (sort){
	    GENESIS_NVTX_RANGE("main_loop.sort_and_sort_slippage");
	    int shift=beam->sort();
	  ...
	  {
	    GENESIS_NVTX_RANGE("main_loop.field_track_all");
	    for (int i=0; i<field->size();i++){
	      GENESIS_NVTX_RANGE("main_loop.field_track_one");
	  ...
	  {
	    GENESIS_NVTX_RANGE("main_loop.field_slippage_all");
	    for (int i=0;i<field->size();i++){
	      GENESIS_NVTX_RANGE("main_loop.field_slippage_one");
	  ...
	  if (und->outstep()) {
	    GENESIS_NVTX_RANGE("main_loop.diagnostics");
	    // Diagnostics decide internally whether compact CUDA reductions are
	    // sufficient.  Do not force a full beam/field host synchronization here.
	...
	{
	  GENESIS_NVTX_RANGE("diagnostics.write_output_file");
