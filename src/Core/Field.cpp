...
void Field::initSolver(bool isFFT,bool filter, double xc, double yc, double sig, bool useCuda) {
    if (hasSolver_) {
        delete solver;
    }
#ifdef GENESIS_CUDA
...
{
  GENESIS_NVTX_RANGE("field.track");
...
#ifdef GENESIS_CUDA
bool Field::syncCudaFieldToHost()
{
  GENESIS_NVTX_RANGE("field.cuda.sync_to_host_if_dirty");
  if (solver == nullptr) {
    return true;
  }
  return solver->syncCudaFieldToHost(this);
}

void Field::markCudaHostFieldDirty()
{
  if (solver != nullptr) {
    solver->markCudaHostFieldDirty();
  }
}

bool Field::copyCudaFieldToBeam(genesis_cuda::BeamState *beamState, std::size_t dstOffset)
{
  if (solver == nullptr) {
    return false;
  }
  return solver->copyCudaFieldToBeam(beamState, this, dstOffset);
}

bool Field::canBuildSourceFromCudaBeam() const
{
  if (solver == nullptr) {
    return false;
  }
  return solver->canBuildSourceFromCudaBeam();
}

bool Field::applyCudaSlippage(int direction)
{
  GENESIS_NVTX_RANGE("field.cuda.slippage_single_rank");
  if (solver == nullptr) {
    return false;
  }
  return solver->applyCudaSlippage(this, direction);
}

bool Field::downloadCudaSlippageSlice(int direction, double *hostBuffer, std::size_t hostDoubles)
{
  GENESIS_NVTX_RANGE("field.cuda.download_slippage_boundary_slice");
  if (solver == nullptr) {
    return false;
  }
  return solver->downloadCudaSlippageSlice(this, direction, hostBuffer, hostDoubles);
}

bool Field::applyCudaSlippageBoundary(int direction, const double *hostBoundary, std::size_t hostDoubles, bool zeroBoundary)
{
  GENESIS_NVTX_RANGE("field.cuda.apply_slippage_boundary");
  if (solver == nullptr) {
    return false;
  }
  return solver->applyCudaSlippageBoundary(this, direction, hostBoundary, hostDoubles, zeroBoundary);
}
#endif
