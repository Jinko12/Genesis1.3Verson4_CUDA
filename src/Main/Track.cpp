...
void Track::usage(){
  ...
  cout << " bool cuda_fieldsolver = false" << endl;
  cout << " bool cuda_beam = <cuda_fieldsolver>" << endl;
...
bool Track::init(int inrank, int insize, map<string,string> *arg, Beam *beam, vector<Field *> *field,Setup *setup, Lattice *lat, AlterLattice *alt,Time *time, FilterDiagnostics &filter)
{
  ...
  if (arg->find("cuda_fieldsolver")!=end) {cudasolver = atob(arg->at("cuda_fieldsolver")); arg->erase(arg->find("cuda_fieldsolver"));}
  cudabeam = cudasolver;
  if (arg->find("cuda_beam")!=end) {cudabeam = atob(arg->at("cuda_beam")); arg->erase(arg->find("cuda_beam"));}
  ...
  // select solver
#ifndef GENESIS_CUDA
  cudasolver=false;   // CUDA solver needs a USE_CUDA build
  cudabeam=false;     // CUDA beam tracking needs a USE_CUDA build
#endif
#ifndef FFTW
  if (!cudasolver) {
    fftsolver=false;  // CPU FFT solver needs FFTW; CUDA FFT uses cuFFT when cuda_fieldsolver=true.
  }
#endif
  beam->setCudaTracking(cudabeam);
  for (int i=0; i<field->size();i++){
    field->at(i)->initSolver(fftsolver,doFilter,xc,yc,sig,cudasolver);
  }
